...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar> *)registrar {
  ...
  BetterPlayerPlugin *instance =
      [[BetterPlayerPlugin alloc] initWithRegistrar:registrar];
...
- (void)setupRemoteCommandNotification:(BetterPlayer *)player, NSString *title,
  ...
  NSMutableDictionary *nowPlayingInfoDict = [@{
    ...
    MPNowPlayingInfoPropertyElapsedPlaybackTime :
        [NSNumber numberWithFloat:positionInSeconds],
    MPMediaItemPropertyPlaybackDuration :
        [NSNumber numberWithFloat:durationInSeconds],
  ...
  if (imageUrl != [NSNull null]) {
    ...
    if (key != [NSNull null]) {
      if (artworkImage) {
        [nowPlayingInfoDict setObject:artworkImage
        ...
        dispatch_async(queue, ^{
          @try {
            ...
            if ([imageUrl rangeOfString:@"http"].location == NSNotFound) {
              ...
              tempArtworkImage = [UIImage
                  imageWithData:[NSData dataWithContentsOfURL:nsImageUrl]];
            }
            if (tempArtworkImage) {
              MPMediaItemArtwork *artworkImage =
                  [[MPMediaItemArtwork alloc] initWithImage:tempArtworkImage];
...
- (void)handleMethodCall:(FlutterMethodCall *)call
  ...
  if ([@"init" isEqualToString:call.method]) {
    ...
    if ([@"setDataSource" isEqualToString:call.method]) {
      ...
      int preferredForwardBufferDurationIos = [dataSource[@"preferredForwardBufferDurationIos"] intValue];
      ...
      // Without https://github.com/flutter/engine/pull/12695, there is no
      // guarantee that the texture has completed the un-reregistration. It may
      // leads a crash if we dispose the `player` before the texture is
      // unregistered. We add a dispatch_after hack to make sure the texture is
      // unregistered before we dispose the `player`.
      //
      // TODO(cyanglaz): Remove this dispatch block when
      // https://github.com/flutter/flutter/commit/8159a9906095efc9af8b223f5e232cb63542ad0b
      // is in stable And update the min flutter version of the plugin to the
      // stable version.
      dispatch_after(
      ...
      if (urlArg != [NSNull null]) {
        ...
        if ([_cacheManager isPreCacheSupportedWithUrl:url
                                       videoExtension:videoExtension]) {
          [_cacheManager stopPreCache:url
                             cacheKey:cacheKey
                    completionHandler:^(BOOL success){
                    }];
        } else {
